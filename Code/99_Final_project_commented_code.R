...
# Load Italy's municipal boundaries (GADM level 3) and convert to 'sf'
italy_admin3 <- st_as_sf(gadm("ITA", level = 3, path = tempdir()))

# Select target municipalities in Sondrio Province
target_municipalities <- c("Livigno", "Valdidentro", "Valdisotto", "Bormio",
                           "Valfurva", "Grosio", "Sondalo", "Grosotto")
selected <- italy_admin3[italy_admin3$NAME_3 %in% target_municipalities, ]
selected <- st_make_valid(selected)  # Fix geometries if needed

# Create bounding box polygon around selected municipalities
bbox <- st_as_sfc(st_bbox(selected))
bbox <- st_sf(geometry = bbox, crs = st_crs(selected))

# Plot bbox and municipalities
plot(st_geometry(bbox), border = "red", lwd = 2, main = "Bounding Box + Selected Municipalities")
plot(st_geometry(selected), add = TRUE, col = rgb(0, 0, 1, 0.3), border = "blue")

# Export bounding box as WKT
st_as_text(st_geometry(bbox))
